...
function extractStepsFromFile(absPath, relPath) {
  ...
  // Nav matrices: soft(item.id, ...) with catalog { id: "OPH-…", label: "…" }
  const navItemRe =
    /\{\s*id:\s*["']([^"']+)["']\s*,\s*label:\s*["']([^"']+)["']/gs;
  ...
  while ((m = navItemRe.exec(text))) {
    const role = /ADMIN_NAV/.test(text)
      ? "Admin"
      : /BRAND_NAV/.test(text)
        ? "Brand"
        : /CLIENT_NAV/.test(text)
          ? "Client"
          : "Nav";
    found.push({
      index: m.index,
...
function main() {
  ...
  for (const flow of config.flows || []) {
    ...
    const isJourney =
      String(flow.id) === "8" || /journey|e2e/i.test(flow.name || "");
  ...
  for (const key of Object.keys(catalog)) {
    total += catalog[key].steps.length;
    console.log(`  Flow ${key}: ${catalog[key].steps.length} steps`);
  }
