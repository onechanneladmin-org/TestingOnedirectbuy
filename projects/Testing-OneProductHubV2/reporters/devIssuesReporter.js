...
function renderMarkdown(issues, meta) {
  ...
  for (const severity of ["critical", "major", "minor", "other"]) {
    ...
    group.forEach((issue, index) => {
      ...
      lines.push(
        issue.suggestion ||
          "Investigate with the screenshot and reproduce the flow.",
      );
      lines.push("");
    });
  }
  ...
  lines.push(`3. Reproduce on https://oneproducthub.com/`);
  lines.push(
    `4. Fix, then re-run: \`npx playwright test tests/OneproducthubV2\``,
  );
  lines.push("");
...
class DevIssuesReporter {
  ...
  async onEnd(result) {
    ...
    const latestMd = path.join(
      "test-results",
      "dev-issues",
      "LATEST_ISSUES_FOR_DEVELOPERS.md",
    );
