...
async function startFlowRun(flowId, opts = {}) {
  ...
  if (process.platform === "win32") {
    ...
    const bat = [
      ...
      `title OPH Flow ${key} — ${occurrenceId.slice(0, 8)}`,
      `cd /d "${rootCmd}"`,
      ...
      'set "OPH_PAUSE_ON_EXIT=0"',
      ...headedLines,
      "echo.",
      "echo ============================================",
      "echo  One Product Hub V2 Flow Control Plane — runner",
      `echo  Flow:       ${key}`,
    ...
    const startCmd = `start "OPH Flow ${key}" /D "${rootCmd}" cmd.exe /c "${batPathCmd}"`;
