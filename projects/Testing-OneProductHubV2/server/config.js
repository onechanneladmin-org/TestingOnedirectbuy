...
module.exports = {
  ...
  get MONGODB_URI() {
    return (
      ...
      "mongodb://127.0.0.1:27017/oneproducthub-tests"
    );
  },
  get MONGODB_DB() {
    return process.env.MONGODB_DB || "oneproducthub-tests";
  },
  /** Shared bearer token; when empty, auth is disabled (local/dev). */
  get API_TOKEN() {
    return process.env.API_TOKEN || process.env.STATUS_API_TOKEN || "";
  },
  /** Base URL the Playwright child uses to POST step updates. */
  get STATUS_API_URL() {
    return process.env.STATUS_API_URL || `http://127.0.0.1:${PORT}`;
  },
  /** Public DNS for Atlas SRV (comma-separated). */
  get DNS_SERVERS() {
    return process.env.DNS_SERVERS || "8.8.8.8,8.8.4.4";
  },
