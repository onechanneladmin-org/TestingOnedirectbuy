 ...
 *   RUNNING_OCCURRENCE_ID env, reports/oph-flow-run.json, reports/oph-active-occurrence.txt
 *
 * Updates:
...
function metaPath() {
  return path.join(projectRoot(), "reports", "oph-flow-run.json");
}

function activePath() {
  return path.join(projectRoot(), "reports", "oph-active-occurrence.txt");
}
...
export async function reportStepStatus(payload) {
  ...
  if (!ctx.occurrenceId) {
    // No flow-runner context — silent (normal playwright test)
    return;
  }
