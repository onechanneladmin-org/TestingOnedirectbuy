/**
 * Soft checks with live step reporting for the flow control UI.
 * Default: report failure then rethrow (hard-fail, ProductHub behavior).
 * Soft-continue only when CI_SOFT_PASS=1.
 */
import fs from "fs";
...
function isSoftPassEnabled() {
  return (
    process.env.CI_SOFT_PASS === "1" || process.env.CI_SOFT_PASS === "true"
  );
}
...
function suggestionFor(marker) {
  switch (marker) {
    ...
    case MARKERS.TIMEOUT:
      return "Element not found or not interactable — UI label/role may have changed; update locator or confirm viewport.";
    case MARKERS.NAVIGATION:
      return "URL did not match expected pattern — confirm route or destination changed.";
    case MARKERS.PERFORMANCE:
      return "Page exceeded the accepted load budget — check CDN/API latency or relax the threshold.";
    case MARKERS.AUTH:
      return "Auth step failed — verify ONEPRODUCTHUB_* credentials in .env / CI secrets.";
    default:
...
export function createSoftChecker(page, testInfo) {
  ...
  /**
   * Run a check; reports live step status for the flow UI.
   * On failure: records issue; rethrows unless CI_SOFT_PASS=1.
   * @param {string} id Step id e.g. OPH-LOGIN-CLIENT-1
   * @param {string} title Short human description
   * @param {() => Promise<void>} fn
   * @param {{ severity?: string; marker?: string; category?: string }} [opts]
