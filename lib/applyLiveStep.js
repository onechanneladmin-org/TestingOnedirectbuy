...
function findByPrefix(list, stepId, title) {
  const exact = (list || []).find((s) => s.stepId === stepId);
  if (exact) return exact;
  const byTitle = (list || []).find(
    (step) => (title && step.title === title) || (stepId && step.title === stepId),
  );
...
function findCatalogParent(steps, stepId, title) {
  return findByPrefix(steps, stepId, title);
}
...
function leafSteps(steps) {
  if (isSingleUseCaseRun(steps)) return steps[0].children || [];
  const leaves = [];
  for (const step of steps || []) {
    if (Array.isArray(step.children) && step.children.length) {
      leaves.push(...step.children);
    } else {
      leaves.push(step);
    }
  }
...
function countCompleted(steps) {
  return leafSteps(steps).filter((s) => TERMINAL.has(s.status)).length;
}

function countTotal(steps) {
  return leafSteps(steps).length;
}

function countByStatus(steps) {
  const leaves = leafSteps(steps);
  const counts = {
    pending: 0,
    running: 0,
    passed: 0,
...
function applyLiveStep(steps, payload, opts = {}) {
  ...
  let parent = findCatalogParent(steps, payload.stepId, payload.title);

  if (parent && Array.isArray(parent.children) && parent.children.length) {
    const child = findByPrefix(parent.children, payload.stepId, payload.title);
    if (child) {
