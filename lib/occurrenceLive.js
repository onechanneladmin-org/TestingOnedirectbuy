...
const { applyLiveStep, countCompleted, countTotal } = require("./applyLiveStep");
...
function writeMeta(meta) {
  ...
  if (meta?.occurrenceId) {
    const uniquePath = path.join(
      ROOT,
      "reports",
      `odb-flow-run-${meta.occurrenceId}.json`,
    );
    try {
      fs.writeFileSync(uniquePath, `${JSON.stringify(meta, null, 2)}\n`, "utf8");
    } catch {
      // ignore
    }
    fs.writeFileSync(
...
async function reportStepToDb(payload) {
  ...
  const steps = Array.isArray(occ.steps)
    ? occ.steps.map((s) => ({
        ...s,
    ...
    : [];
  const now = new Date();
  const result = applyLiveStep(steps, payload, { now });
  if (result.skipped) {
    return { ok: true, skipped: true };
  }

  const stepsCompleted = countCompleted(steps);
  const currentStepId =
    payload.status === "running"
      ? result.currentStepId
      : occ.currentStepId === result.step?.stepId
        ? null
        : occ.currentStepId;
  ...
  await col.updateOne(
    { occurrenceId },
    {
      $set: {
        steps,
        stepsCompleted,
        stepsTotal: Math.max(occ.stepsTotal || 0, countTotal(steps)),
        currentStepId,
        updatedAt: now,
      },
    },
